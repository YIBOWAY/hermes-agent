...
class TestStartRun:
    ...
    @pytest.mark.asyncio
    async def test_start_does_not_invent_delegation_wake_target(self, adapter):
        """/v1/runs has no persistent callback channel of its own.

        A managed Hermes session id is transcript authority, not a routable
        chat target, so background delegation must remain synchronous unless a
        future API contract introduces an explicit wake channel.
        """
        app = _create_runs_app(adapter)
        captured = {}
        ...
        assert captured.get("origin_session_id") == "", (
            "runs route must not counterfeit a routable chat wake target"
        )
