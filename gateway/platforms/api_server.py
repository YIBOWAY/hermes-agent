...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        try:
            resolved_source_id = await asyncio.to_thread(
                db.resolve_resume_session_id, source_id
            )
            resolved_source = await asyncio.to_thread(
                db.get_session, resolved_source_id
            )
            if resolved_source is None:
                raise LookupError("resolved source session is missing")
            source_messages = await asyncio.to_thread(
                db.get_messages, resolved_source_id
            )
        except Exception:
            logger.exception(
        ...
        try:
            ...
            title = await asyncio.to_thread(db.sanitize_title, str(title))
        except ValueError as exc:
            return web.json_response(_openai_error(str(exc), code="invalid_title"), status=400)
        ...
        try:
            ...
            await asyncio.to_thread(db.replace_messages, fork_id, prefix)
            await asyncio.to_thread(db.set_session_title, fork_id, title)
        except Exception:
            logger.exception(
    ...
    async def _handle_stop_run(self, request: "web.Request") -> "web.Response":
        ...
        if agent is not None:
            ...
            if stop_delivery_failed and self._broker_enabled():
                return self._durable_unavailable_response()
